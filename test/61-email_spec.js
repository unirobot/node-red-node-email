...
var emailNode = require('../61-email.js');
...
describe('email Node', function () {
    ...
    describe("send and receive", () =>{
        it('send email (custom)', function (done) {
            var flow = [
            {
                id: "n1",
                type: "e-mail",
                name: settings.smtp.send_address,
            ...
            var credential = {
                "n1": {
                    userid : settings.smtp.credential.userid,
                    password : settings.smtp.credential.password
                }
            };
            helper.load(emailNode, flow, credential, function () {
                var n1 = helper.getNode("n1");
                n1.receive({payload: "test", topic: "send test mail"});
                n1.on('call:status', call => {
                    if(call.lastArg.fill === "blue"){
                        call.lastArg.should.have.property("text","email.status.sending");
                        done();
                    }
                });
            });
        ...
        it('receive email (custom)', function (done) {
            var flow = [
                {
                    id: "n2",
                    type: "e-mail in",
            ...
            helper.load(emailNode, flow, credential, function () {
                ...
                n3.on("input", (msg) => {
                    if(msg.topic === "send test mail"){
                        msg.payload.should.have.startWith("test");
                        done();
                    }
                });
            });
